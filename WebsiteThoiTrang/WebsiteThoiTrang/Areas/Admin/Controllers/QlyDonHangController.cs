...
namespace WebsiteThoiTrang.Areas.Admin.Controllers
{
    public class QlyDonHangController : BaseController
    {
        ...
        public ActionResult DanhSachDonHang(string timkiem, int? page)
        {
            ...
            if (timkiem != null)
            {
                List<DONHANG> listKQ = db.DONHANG.Where(n => n.MaDH.ToString().Contains(timkiem) ||  n.NHANVIEN.TenNV.ToString().Contains(timkiem) ||  n.KHACHHANG.TenKH.ToString().Contains(timkiem)).ToList();
                if (listKQ.Count == 0)
                {
                    TempData["thongbao"] = "Không tìm thấy đơn hàng nào phù hợp.";
                    return View(db.DONHANG.Where(n => n.TinhTrang != "Chưa duyệt").OrderByDescending(n => n.MaDH).ToPagedList(pageNumber, pageSize));
                }
        ...
        public ActionResult DanhSachChuaDuyet(string timkiem, int? page)
        {
            ...
            ViewBag.TuKhoa = timkiem;
                if (timkiem != null)
                {
                    List<DONHANG> listKQ = db.DONHANG.Where(n => n.MaDH.ToString().Contains(timkiem) && n.TinhTrang == "Chưa duyệt" || n.NHANVIEN.TenNV.ToString().Contains(timkiem) && n.TinhTrang == "Chưa duyệt" || n.KHACHHANG.TenKH.ToString().Contains(timkiem) && n.TinhTrang == "Chưa duyệt").ToList();
                    if (listKQ.Count == 0)
                    {
                        TempData["thongbao"] = "Không tìm thấy đơn hàng nào phù hợp.";
                        return View(db.DONHANG.Where(n => n.TinhTrang == "Chưa duyệt").OrderByDescending(n => n.MaDH).ToPagedList(pageNumber, pageSize));
        ...
        public ActionResult DonHangChoGiao(string timkiem, int? page)
        {
            ...
            if (timkiem != null)
            {
                ...
                {
                    TempData["thongbao"] = "Không tìm thấy đơn hàng nào phù hợp.";
                    return View(db.DONHANG.Where(n => n.NgayGiao.ToString()=="" && n.TinhTrang=="Đã duyệt").OrderByDescending(n => n.MaDH).ToPagedList(pageNumber, pageSize));
                }
        ...
        public ActionResult DuyetDonHang(int madh)
        {
            ...
            DONHANG dh = db.DONHANG.Where(n => n.MaDH == madh).SingleOrDefault();
            dh.MaNV = manv;
        ...
        {
            int manv = (int)Session["MaNV"];
            List<CHITIETHD> cthd = db.CHITIETHD.Where(n => n.MaDH == madh).ToList();
            DONHANG dh = db.DONHANG.Where(n => n.MaDH == madh).SingleOrDefault();
            dh.MaNV = manv;
            dh.TinhTrang = "Đã hủy";
        ...
        public ActionResult ChiTietDH(int madh, string timkiem, int? page)
        {
            ...
            if (timkiem != null)
            {
                List<CHITIETHD> listKQ = db.CHITIETHD.Where(n => n.MaDH.ToString().Contains(timkiem)).ToList();
                if (listKQ.Count == 0)
                {
                    TempData["thongbao"] = "Không tìm thấy đơn hàng nào phù hợp.";
                    return View(db.CHITIETHD.Where(n => n.MaDH==madh).OrderByDescending(n => n.MaDH).ToPagedList(pageNumber, pageSize));
                }
