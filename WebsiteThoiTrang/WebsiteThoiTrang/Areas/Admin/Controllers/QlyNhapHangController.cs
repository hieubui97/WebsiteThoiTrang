...
namespace WebsiteThoiTrang.Areas.Admin.Controllers
{
    ...
    public class QlyNhapHangController : BaseController
    {
        ...
        public ActionResult DanhSachPhieuNhap(string timkiem, int? page)
        {
            ...
            if (timkiem != null)
            {
                List<PHIEUNHAP> listKQ = db.PHIEUNHAP.Where(n => n.MaPN.ToString().Contains(timkiem)).ToList();
                if (listKQ.Count == 0)
                {
                    TempData["thanhcong"] = "Không tìm thấy phiếu nhập nào phù hợp.";
                    return View(db.PHIEUNHAP.OrderByDescending(n => n.MaPN).ToPagedList(pageNumber, pageSize));
                }
        ...
        public ActionResult ChiTietPN(int mapn, string timkiem, int? page)
        {
            ...
            if (timkiem != null)
            {
                List<CHITIETPN> listKQ = db.CHITIETPN.Where(n => n.MaPN.ToString().Contains(timkiem)).ToList();
                if (listKQ.Count == 0)
                {
                    TempData["thongbao"] = "Không tìm thấy đơn hàng nào phù hợp.";
                    return View(db.CHITIETPN.Where(n => n.MaPN == mapn).OrderByDescending(n => n.MaPN).ToPagedList(pageNumber, pageSize));
                }
        ...
        {
            ViewBag.TenNCC = new SelectList(db.NCC.ToList().OrderBy(n => n.TenNCC), "MaNCC", "TenNCC");
            return View();
        }
        ...
        public ActionResult ThemMoiCTPN(int mapn)
        {
            ...
            ViewBag.TenMau = new SelectList(db.MAUSAC.ToList().OrderBy(n => n.Color), "MaMau", "Color");
            ViewBag.TenSize = new SelectList(db.SIZE.ToList().OrderBy(n => n.Size1), "MaSize", "Size1");
            return View();
        }
        ...
        public ActionResult ThemMoiCTPN(CHITIETPN ctpn)
        {
            ...
            ViewBag.TenMau = new SelectList(db.MAUSAC.ToList().OrderBy(n => n.Color), "MaMau", "Color");
            ViewBag.TenSize = new SelectList(db.SIZE.ToList().OrderBy(n => n.Size1), "MaSize", "Size1");
            //var ctpnkt = db.CHITIETPN.Where(n => n.MaSP == masp && n.MaSize == masize && n.MaMau == mamau).Count();
            CHITIETSP ctsp = db.CHITIETSP.SingleOrDefault(n => n.MaSP == masp && n.MaSize == masize && n.MaMau == mamau);
        ...
        {
            int manv = (int)Session["MaNV"];
            List<CHITIETPN> ctpn = db.CHITIETPN.Where(n => n.MaPN == mapn).ToList();
            PHIEUNHAP pn = db.PHIEUNHAP.Where(n => n.MaPN == mapn).SingleOrDefault();
            pn.MaNV = manv;
            pn.TinhTrang = "Đã hủy";
            foreach (var item in ctpn)
            {
                db.CHITIETPN.Remove(item);
                db.SaveChanges();
            }
