...
namespace WebsiteThoiTrang.Areas.Admin.Controllers
{
    public class QlyMauSacController : BaseController
    {
        ...
        public ActionResult DanhSachMauSac(string timkiem, int? page)
        {
            ...
            if (timkiem != null)
            {
                List<MAUSAC> listKQ = db.MAUSAC.Where(n => n.Color.Contains(timkiem)).ToList();
                if (listKQ.Count == 0)
                {
                    TempData["thongbao"] = "Không tìm thấy màu nào phù hợp.";
                    return View(db.MAUSAC.OrderBy(n => n.MaMau).ToPagedList(pageNumber, pageSize));
                }
        ...
        {
            MAUSAC mau = db.MAUSAC.SingleOrDefault(n => n.MaMau == mamau);
            return View(mau);
        }
