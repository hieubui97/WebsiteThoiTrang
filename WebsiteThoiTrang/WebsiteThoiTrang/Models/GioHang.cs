...
namespace WebsiteThoiTrang.Models
{
    public class GioHang
    {
        ...
        {
            masp = Masp;
            //var giay = (from s in db.SANPHAM
            //            join c in db.CHITIETSP
            ...
            soluong = 1;
            CHITIETSP ctgiay = db.CHITIETSP.SingleOrDefault(s =>s.MaSP == Masp && s.MaMau == Mamau && s.MaSize == Masize);
            mamau = ctgiay.MaMau;
            masize = ctgiay.MaSize;
            MAUSAC mausac = db.MAUSAC.SingleOrDefault(m => m.MaMau == mamau);
            tenmau = mausac.Color;
            SIZE sizes = db.SIZE.SingleOrDefault(s => s.MaSize == masize);
            tensize = sizes.Size1;
        }
